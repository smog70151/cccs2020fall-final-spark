...
def main():
    ...
    spDF = sqlContext.createDataFrame(pd.DataFrame(np.array(temp[:10]), columns=["_c"+str(i) for i in range(751)]))
    #spDF.show()
    #spDF = spark.createDataFrame(pd.DataFrame(np.array(temp)))
    # Read in mnist_train.csv dataset
    df = spark.read.option("inferSchema", "true").csv('datas/trains.csv', header=False).orderBy(rand()).repartition(2)
    ...
    #spDF = vector_assembler.transform(spDF)
    predictions = loaded_pipeline.transform(spDF).persist()
    # pdb.set_trace()
    # predictions.take(1)[0].predictions # Take specific index value
    predictions.select("predictions").show() # Take whole
    predictions.select("predictions").collect()[0].predictions # [index for row] 
    pdb.set_trace()
